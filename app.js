...
function process3DayData(rawData, timezone) {
    ...
    rawData.forEach(entry => {
        // entry.time_tag "YYYY-MM-DD HH:MM:SS" (or T format)
        // entry.kp (number or string)

        const dateObj = new Date(entry.time_tag + 'Z'); // Treat as UTC
        ...
        dataPoints.push(parseFloat(entry.kp));
    });
...
async function fetch3DayForecast() {
    try {
        ...
        for (let i = 0; i < rawData.length; i++) {
            // entry.observed is status ("observed", "estimated", "predicted")
            if (rawData[i].observed !== 'observed') {
